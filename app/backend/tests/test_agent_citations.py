...
def test_papers_from_hybrid_search():
    result = json.dumps([{"id": 3, "title": "T", "doi": "10.1/x", "score": 1.0}])
    papers = agent._papers_from_tool_result("hybrid_search", result)
    assert papers == [{"doi": "10.1/x", "title": "T", "paper_id": 3}]


def test_papers_from_paper_details():
    result = json.dumps({"id": 5, "title": "Deep", "doi": "10.2/y", "abstract": "..."})
    papers = agent._papers_from_tool_result("get_paper_details", result)
    assert papers == [{"doi": "10.2/y", "title": "Deep", "paper_id": 5}]
...
def test_build_citations_separates_verified_and_unverified():
    retrieved = {"10.1/x": {"doi": "10.1/x", "title": "T", "paper_id": 3}}
    text = "Evidence in [Author, 10.1/x] but also 10.9/hallucinated."
    citations, unverified = agent._build_citations(text, retrieved)
    assert citations == [{"doi": "10.1/x", "title": "T", "paper_id": 3}]
    assert unverified == ["10.9/hallucinated"]
